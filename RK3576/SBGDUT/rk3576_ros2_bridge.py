#!/usr/bin/env python3
"""ROS 2 Jazzy RK3576-side bridge for the phase-two link test.
...
class Rk3576Bridge(Node):
    def __init__(self, outgoing: "asyncio.Queue[Dict[str, Any]]") -> None:
        super().__init__("rk3576_public_ws_bridge")
        self.outgoing = outgoing
        self.sequence = 0
        self.publisher = self.create_publisher(String, "/pc_to_cat", 10)
        self.subscription = self.create_subscription(
            Vector3, "/cat_to_pc", self.on_local_message, 10
        )
    ...
    def on_local_message(self, message: Vector3) -> None:
        ...
        frame = {
            ...
            "data": {"x": message.x, "y": message.y, "z": message.z},
        }
        if self.outgoing.full():
            self.outgoing.get_nowait()
        self.outgoing.put_nowait(frame)
        self.get_logger().info(f"排队发送 /cat_to_pc seq={self.sequence}")
    ...
    def publish_remote(self, frame: Any) -> None:
        ...
        if (
            ...
            or frame.get("topic") != "/pc_to_cat"
            or frame.get("msg_type") != "std_msgs/msg/String"
        ):
            raise ValueError("不是允许的 PC Topic 帧")
        data = frame.get("data")
        if not isinstance(data, dict) or not isinstance(data.get("data"), str):
            raise ValueError("String.data 必须是字符串")
        self.publisher.publish(String(data=data["data"]))
        self.get_logger().info(f"已发布 /pc_to_cat seq={frame.get('seq', '?')}")
...
async def receive_frames(websocket: Any, node: Rk3576Bridge) -> None:
...
async def connected_session(
    websocket: Any, node: Rk3576Bridge, outgoing: "asyncio.Queue[Dict[str, Any]]"
) -> None:
...
async def websocket_loop(
    uri: str, node: Rk3576Bridge, outgoing: "asyncio.Queue[Dict[str, Any]]"
) -> None:
    while rclpy.ok():
        try:
